class GoogleApi
	...
	def get_photo(key, photoreference)
		...
		uri = URI("http://maps.googleapis.com/maps/api/place/photo?maxwidth=400&maxheight=400&photoreference=" + photoreference + "&sensor=false&key=" + key)
		res = Net::HTTP.get(uri)
		#puts JSON.parse(res.force_encoding('UTF-8'))
	end

	def google_search_image(term)
		GoogleAjax::Search.images(term.to_s)
	end
end
